...
class ChefTest < ActiveSupport::TestCase
  ...
  test 'Valid email addresses should be accepted' do
    valid_addresses = %w(user@user.com R_TDD-S@three.part.org
                         first.last@corp.com user+name@any.com)
  ...
  test 'Invalid email addresses must be rejected' do
    invalid_addresses = %w(user@user, R_TDD-S@three. first.last@corp@corp.com
                           user_at_eee_org user@example,com)
